...
$firebaseObject(ref.push());$scope.data.$save().then(function(){})};$scope.sendMsg=function(){$scope.data.$save().then(function(){console.log("Feedback Sent");Dialog.close()}).catch(function(error){console.log("Error!")})}});app.controller("search",function($scope,$firebaseObject){$scope.posts=[];var ref=firebase.database().ref("data/posts").once("value").then(function(snapshot){var obj=snapshot.val();angular.forEach(obj,function(val){$scope.posts.push(val)});console.log($scope.posts)})});
