...
app.controller("feedback",function($scope,$firebaseObject,$firebaseAuth,Dialog){var auth=$firebaseAuth();var ref=firebase.database().ref("feedback");$scope.close=function(){Dialog.close()};$scope.showMsg=false;$scope.signIn=function(){$scope.showMsg=true;$scope.firebaseUser=null;auth.$signInAnonymously().then(function(firebaseUser){$scope.firebaseUser=firebaseUser;$scope.data=$firebaseObject(ref.push())}).catch(function(error){console.log("Error occured during sending")})};$scope.thumbs=function(e){$scope.data=
$firebaseObject(ref.push());$scope.data.$save().then(function(){})};$scope.sendMsg=function(){$scope.data.$save().then(function(){console.log("Feedback Sent");Dialog.close()}).catch(function(error){console.log("Error!")})}});app.controller("search",function($scope,$firebaseObject){$scope.posts=[];var ref=firebase.database().ref("data/posts").once("value").then(function(snapshot){var obj=snapshot.val();angular.forEach(obj,function(val){$scope.posts.push(val)})})});
app.controller("share",function($scope,Dialog,Toast,$mdMedia,$mdBottomSheet){$scope.link=window.location.href;$scope.title=angular.element(window.document)[0].title;$scope.Toast=Toast;$scope.openShare=function(ev){if($mdMedia("xs")==true){$mdBottomSheet.show({templateUrl:"/assets/sharebtm-template.html",clickOutsideToClose:true,controller:"share"});return}Dialog.show("share",ev)};$scope.closeShare=function(){Dialog.close()}});app.controller("ProjectCtrl",function(Panel){this.show=function(dest){Panel(dest)}});
...
app.directive("tileImage",function(){return{restrict:"E",template:"\x3cimg ng-src\x3d'{{::source}}' style\x3d'opacity:{{::opacity}}; width: {{::width}};'/\x3e",scope:{source:"@",opacity:"@",width:"@"}}});app.directive("tileHeader",function(){return{restrict:"E",transclude:true,template:"\x3cspan class\x3d'tile-title' ng-transclude\x3e\x3c/span\x3e\x3cbr/\x3e\x3cspan style\x3d'opacity: 0.67; margin-top: 4px;'\x3e{{::tag}}\x3c/span\x3e",scope:{name:"@",tag:"@"}}});
