...
function($event){if($scope.gridLay==true){$scope.gridLay=false;if(!navigator.onLine)Toast("You're offline. Serving from cache!")}};$scope.go=function(dest){$window.location.href=$sce.trustAsResourceUrl(dest)}});
app.controller("fireCtrl",function($scope,$firebaseObject,$firebaseAuth){var auth=$firebaseAuth();var ref=firebase.database().ref("feedback");$scope.showMsg=false;$scope.signIn=function(){$scope.showMsg=true;$scope.firebaseUser=null;auth.$signInAnonymously().then(function(firebaseUser){$scope.firebaseUser=firebaseUser;$scope.data=$firebaseObject(ref.push())}).catch(function(error){console.log("Error occured during sending")})};$scope.sendMsg=function(){$scope.data.$save().then(function(){console.log("Feedback Sent")}).catch(function(error){console.log("Error!")})}});
...
app.controller("shareCtrl",function($scope,$mdDialog,$location,Toast){$scope.copy=true;$scope.title=angular.element(window.document)[0].title;$scope.url=$location.absUrl();$scope.openShare=function(ev){var $jscomp$this=this;if(navigator.share!==undefined){navigator.share({title:$scope.title,text:$scope.title,url:$scope.url}).then(function(){return Toast("Thanks for sharing!")},function(error){return Toast("Error in sharing.")});return}$mdDialog.show({templateUrl:"/assets/share-template.html",parent:angular.element(document.body),
targetEvent:ev,controller:function(){return $jscomp$this},controllerAs:"share",clickOutsideToClose:true})};$scope.close=function(){$mdDialog.cancel()}});app.controller("ModalController",ModalController);app.controller("ModalCtrl",ModalCtrl);function ModalController($mdPanel){this._mdPanel=$mdPanel}
ModalController.prototype.showPanel=function(dest){var tmpl="/project/"+dest+".html";var position=this._mdPanel.newPanelPosition().absolute().center();var animation=this._mdPanel.newPanelAnimation().withAnimation(this._mdPanel.animation.FADE);var config={animation:animation,attachTo:angular.element(document.body),controller:ModalCtrl,controllerAs:"ctrl",disableParentScroll:this.disableParentScroll,templateUrl:tmpl,hasBackdrop:true,panelClass:"modal-container",position:position,trapFocus:true,zIndex:150,
clickOutsideToClose:true,escapeToClose:true,focusOnOpen:true};this._mdPanel.open(config)};function ModalCtrl(mdPanelRef){this._mdPanelRef=mdPanelRef}ModalCtrl.prototype.closePanel=function(){var panelRef=this._mdPanelRef;panelRef&&panelRef.close().then(function(){angular.element(document.querySelector(".share")).focus();panelRef.destroy()})};
