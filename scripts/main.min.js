...
$firebaseObject(ref.push());$scope.data.$save().then(function(){})};$scope.sendMsg=function(){$scope.data.$save().then(function(){console.log("Feedback Sent");Dialog.close()}).catch(function(error){console.log("Error!")})}});app.controller("share",function($scope,Dialog,Toast){$scope.link=window.location.href;$scope.title=angular.element(window.document)[0].title;$scope.Toast=Toast;$scope.openShare=function(ev){Dialog.show("share",ev)};$scope.closeShare=function(){Dialog.close()}});
app.controller("ProjectCtrl",function(Panel){this.show=function(dest){Panel(dest)}});app.factory("Toast",["$mdToast","$window",function($mdToast,$window){return function(msg,action){if(action!==""){var toast=$mdToast.simple().textContent(msg).action(action).highlightAction(true);$mdToast.show(toast).then(function(response){if(response=="ok")switch(action){case "refresh":$window.location.reload();break;case "ok":$mdToast.hide();break}},function(err){angular.noop()})}else $mdToast.showSimple(msg)}}]);
