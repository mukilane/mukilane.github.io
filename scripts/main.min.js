...
app.controller("main",["$scope","$interval","$window","Toast","$sce","Dialog",function(a,d,b,c,e,f){a.gridLay=!0;localStorage.getItem("theme")?(a.isDark=!0,a.theme={bg:"grey-800",footer:"grey-700"}):(a.isDark=!1,a.theme={bg:"grey-A100",footer:"grey-A100"});a.setDark=function(b){(a.isDark=b)?(localStorage.setItem("theme","dark"),c("Dark theme activated","refresh")):(localStorage.removeItem("theme"),c("Light theme activated","refresh"))};a.isHomePage=function(){return"/"===window.location.pathname?
...
app.controller("HdocCtrl",["$scope","$http",function(a,d){a.date=new Date;this.myDate=new Date;this.minDate=new Date(this.myDate.getFullYear(),this.myDate.getMonth()-2,this.myDate.getDate());this.maxDate=new Date(this.myDate.getFullYear(),this.myDate.getMonth()-1,this.myDate.getDate())}]);
