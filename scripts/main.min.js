...
app.run(function(){localStorage.getItem("theme")?($scope.isDark=!0,$scope.theme={bg:"grey-800",footer:"grey-700"}):($scope.isDark=!1,$scope.theme={bg:"grey-50",footer:"grey-200"})});
app.controller("main",["$scope","$interval","$window","Toast","$sce","Dialog",function(a,d,b,c,e,f){a.gridLay=!0;a.setDark=function(b){(a.isDark=b)?(localStorage.setItem("theme","dark"),c("Dark theme activated","refresh")):(localStorage.removeItem("theme"),c("Light theme activated","refresh"))};a.counter=0;a.adjective="A Web Developer;A Programmer;Google Play Rising Star;Loves OpenSource;A Linux Admin;Elon Musk Fan;Philomath;Tech Enthusiast".split(";");d(function(){7>a.counter?a.counter++:a.counter=
0},5E3);b.addEventListener("offline",function(){c("You're Offline. Serving from cache!",!1);b.addEventListener("online",function(a){c("You're Online now !","ok")},{once:!0,capture:!1})},!1);a.trust=function(a){e.trustAsUrl(a)};a.show=function(b){1==a.gridLay&&(a.gridLay=!1,navigator.onLine||c("You're offline. Serving from cache!"))};a.showDlg=function(a){f.show("feedback",a)};a.go=function(a){b.location.href=e.trustAsResourceUrl(a)}}]);
...
c.close()}).catch(function(a){console.log("Error!")})}}]);app.controller("search",["$scope","$firebaseObject",function(a,d){a.posts=[];firebase.database().ref("data/posts").once("value").then(function(b){b=b.val();angular.forEach(b,function(b){a.posts.push(b)})})}]);
...
panelClass:"modal-container",position:c,trapFocus:!0,zIndex:150,clickOutsideToClose:!0,escapeToClose:!0,focusOnOpen:!0};this._mdPanel.open(b)}}]);app.directive("tile",function(){return{restrict:"E",transclude:!0,template:"\x3cdiv class\x3d'tile-wrap' style\x3d'background-image: url({{::image}}); opacity: {{::opacity}}; background-position: {{::pos}}; background-size: {{::size}}' ng-transclude\x3e\x3c/div\x3e",scope:{image:"@",opacity:"@",pos:"@",size:"@"}}});
app.directive("imageTile",function(){return{restrict:"E",transclude:!0,template:"\x3cdiv class\x3d'tile-wrap' style\x3d'background-image: url({{::image}}); opacity: {{::opacity}}; background-position: {{::pos}}; background-size: {{::size}}'\x3e\n\t\t\t\t\t\t\t\t\x3cdiv class\x3d'image-tile' ng-transclude\x3e\x3c/div\x3e\n\t\t\t\t\t\t\t\x3c/div\x3e",scope:{image:"@",opacity:"@",pos:"@",size:"@"}}});
